...
- **`/patient/search` is paginated** (`limit`/`offset`, default 20, capped at 100 regardless of what's requested) and returns a `{ patients, total, limit, offset }` envelope rather than a bare array — an unbounded query against a real hospital's patient table would be a real problem, not a hypothetical one. `limit=0` is a deliberately supported case (zero rows, real `total`), distinct from "unspecified" (`-1` internally, defaults to 20) — this distinction is what stops an explicit `limit=0` from silently defaulting instead of doing what it says. Results are ordered by `created_at, id` specifically because Postgres gives zero ordering guarantee under `LIMIT`/`OFFSET` without an explicit `ORDER BY` — without it, two consecutive page requests aren't guaranteed to be stable or non-overlapping.
